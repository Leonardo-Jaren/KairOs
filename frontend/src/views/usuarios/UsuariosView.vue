  ...
  <div class="flex flex-col gap-6">
    ...
    <section class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      <form class="grid gap-3 md:grid-cols-[minmax(220px,1fr)_180px_160px_auto]" @submit.prevent="applyFilters">
        <BaseInput id="users-search" v-model="filters.search" appearance="light"
          placeholder="Buscar por nombre, correo, usuario o DNI">
          <template #icon>
            <Search :size="17" />
          </template>
        </BaseInput>
        <BaseSelect id="users-role" v-model="filters.rol" :options="availableRoleOptions"
          placeholder="Todos los roles" />
        <BaseSelect id="users-status" v-model="filters.activo" :options="[
    ...
    <BaseTable :columns="columns" :items="usuarios" :loading="loading"
      ...
      <template #cell-rol="{ item }">
        <span class="inline-flex rounded-full px-2.5 py-1 text-xs font-semibold ring-1 ring-inset"
          :class="roleClasses[item.rol]">
      ...
      <template #cell-acciones="{ item }">
        <div class="flex justify-end gap-1">
          <button type="button" class="rounded-lg p-2 text-slate-400 hover:bg-slate-100 hover:text-slate-700"
            aria-label="Ver detalle" @click="detailUsuario = item">
            <Eye :size="17" />
          </button>
          <template v-if="canManageAll">
            <button type="button" class="rounded-lg p-2 text-slate-400 hover:bg-primary-50 hover:text-primary-600"
              aria-label="Editar usuario" @click="openEdit(item)">
              <Pencil :size="17" />
            </button>
            <button type="button" class="rounded-lg p-2 text-slate-400 hover:bg-danger-50 hover:text-danger-600"
              aria-label="Desactivar usuario" @click="askDelete(item)">
              <UserX :size="17" />
            </button>
          </template>
        </div>
    ...
    <BasePagination :page="filters.page" :total-pages="pagination.totalPages" :total="pagination.total"
      :loading="loading" @change="changePage" />

    <BaseModal :open="modalOpen" :title="isEditing ? 'Editar usuario' : 'Nuevo usuario'"
      description="Completa los datos institucionales y define sus permisos de acceso." @close="closeModal">
      <form id="user-form" class="grid gap-4 sm:grid-cols-2" @submit.prevent="submit">
        <BaseInput id="user-name" v-model="form.nombre" appearance="light" label="Nombre" placeholder="Ej. Andrea"
          :error="formErrors.nombre" />
        <BaseInput id="user-lastname" v-model="form.apellido" appearance="light" label="Apellido"
          placeholder="Ej. Salazar" />
        <BaseInput id="user-username" v-model="form.username" appearance="light" label="Nombre de usuario"
          placeholder="asalazar" :error="formErrors.username" />
        <BaseInput id="user-dni" v-model="form.dni" appearance="light" label="DNI" placeholder="12345678"
          :error="formErrors.dni" />
        <div class="sm:col-span-2">
          <BaseInput id="user-email" v-model="form.correo" type="email" appearance="light" label="Correo institucional"
            placeholder="usuario@udh.edu.pe" :error="formErrors.correo" autocomplete="email" />
        </div>
        <BaseSelect id="user-role" v-model="form.rol" label="Rol" :options="availableRoleOptions"
          :error="formErrors.rol" />
        <BaseInput id="user-password" v-model="form.password" type="password" appearance="light"
          :label="isEditing ? 'Nueva contraseña (opcional)' : 'Contraseña'" placeholder="Mínimo 8 caracteres"
    ...
    <BaseModal :open="deleteModalOpen" title="Desactivar usuario" size="sm" @close="cancelDelete">
      <p class="text-sm leading-6 text-slate-600">
        ...
        pero se
        conservarán sus relaciones e historial.
      </p>
      <template #footer>
    ...
    <EntityDetailModal :open="detailUsuario !== null" :title="detailUsuario?.nombre_completo ?? ''"
      description="Información de la cuenta y registro de auditoría." modulo="usuario" :object-id="detailUsuario?.id"
      ...
      <template #info>
        <div v-if="detailUsuario" class="grid grid-cols-1 gap-x-8 gap-y-5 sm:grid-cols-2">
          ...
          <div class="flex flex-col gap-0.5">
            ...
            <span class="inline-flex w-fit rounded-full px-2.5 py-1 text-xs font-semibold ring-1 ring-inset"
          ...
          <div class="flex flex-col gap-0.5">
            ...
            <span class="inline-flex w-fit items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-semibold"
