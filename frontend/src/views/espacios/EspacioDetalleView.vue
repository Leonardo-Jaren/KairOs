...
import {
  ...
  CircleDot, GraduationCap, LayoutGrid, MonitorCog, Pencil, RotateCcw, Save, Wrench,
} from '@lucide/vue';
...
const {
  ...
  statusSummary, teacherEquipment, selectedPosition, selectedPositionId, selectedEquipo,
  reportOpen, reportSaving, reportForm, reportErrors, technicianOptions, toast,
  startEditing, cancelEditing, reflowPositions, addRow, removeRow, handleCellClick,
  handleDrop, toggleTeacher, saveLayout, openReport, closeReport, submitReport, closeToast,
...
<template>
  <div class="flex flex-col gap-6">
    ...
    <div v-if="loading" class="h-[520px] animate-pulse rounded-3xl bg-slate-200" />
    ...
    <template v-else-if="espacio">
      <header class="grid gap-4 xl:grid-cols-[minmax(0,1fr)_auto] xl:items-end">
        <div>
        ...
        <div class="flex flex-wrap gap-2">
          <template v-if="editing">
            ...
            <BaseButton variant="accent" :loading="saving" :full-width="false" @click="saveLayout"><template #icon><Save :size="17" /></template>Guardar distribución</BaseButton>
          </template>
          <BaseButton v-else-if="canEdit && equipos.length" variant="accent" :full-width="false" @click="startEditing"><template #icon><Pencil :size="17" /></template>Editar plano</BaseButton>
        </div>
      </header>
      ...
      <section v-if="editing" class="rounded-2xl border border-primary-200 bg-primary-50/60 p-4 shadow-sm">
      ...
      <section class="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
        ...
        <div v-if="equipos.length" class="overflow-x-auto bg-[linear-gradient(#f8fafc_1px,transparent_1px),linear-gradient(90deg,#f8fafc_1px,transparent_1px)] bg-[size:24px_24px] p-4 sm:p-6">
          ...
          <div class="grid gap-3" :style="gridStyle()">
            <div v-for="cell in cells" :key="`${cell.row}-${cell.column}`" class="min-h-27 rounded-2xl p-1.5 text-left transition-colors" :class="[editing ? 'border border-dashed border-slate-300 bg-white/55 hover:border-primary-300 hover:bg-primary-50/50' : cell.equipment ? 'bg-transparent' : 'bg-white/20', editing && selectedPositionId && !cell.equipment ? 'cursor-cell' : '']" @dragover.prevent @drop.prevent="handleDrop($event, cell)">
        ...
        <div v-else class="px-6 py-20 text-center"><MonitorCog :size="38" class="mx-auto text-slate-300" /><p class="mt-4 font-semibold text-slate-700">Este espacio todavía no tiene equipos asignados.</p><p class="mt-1 text-sm text-slate-400">Asigna equipos desde el módulo de inventario para comenzar a construir el plano.</p><BaseButton class="mt-5" variant="accent" :full-width="false" @click="$router.push('/equipos')">Ir a equipos</BaseButton></div>
      </section>
    </template>

    <EquipoPlanoPanel :equipment="selectedEquipo" @close="selectedEquipo = null" @report="openReport" />
